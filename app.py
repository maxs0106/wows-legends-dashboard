...
def main():
    ...
    st.markdown("<hr style='border:1px solid #0D2B36; margin: 25px 0;'>", unsafe_allow_html=True)
    ...
    with t_best:
        if not mode_filtered_ship_df.empty:
            ...
            exp_col = None
            for col in ['MAX_ORIGINAL_EXP', 'MAX_XP', 'MAX_EXP', 'MAX_EXP_DEALT']:
                if col in mode_filtered_ship_df.columns:
                    exp_col = col
                    break
            ...
            best_targets = [
                ...
                ("最高取得経験値", exp_col or "MAX_ORIGINAL_EXP"),
                ("最高撃沈数", "MAX_FRAGS")
            ]
            if plane_col:
            ...
            for label, col_name in best_targets:
                if col_name and col_name in mode_filtered_ship_df.columns:
            ...
            if best_records:
                # 記録数分のカラムを作成して横に並べる
                cols = st.columns(len(best_records))
